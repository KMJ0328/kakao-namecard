...
// SVG 아이콘 (24x24 기본, scale로 크기 조절)
function icon(type, x, y, color = '#111', scale = 1.2) {
  const s = scale;
  const icons = {
...
// 연락처 4줄 (전화/이메일/웹/주소) 헬퍼
function contactBlock(d, startX, startY, iconColor = '#111', textColor = '#333') {
  const gap = 45;
  let svg = '';
  let y = startY;
  if (d.phone) { svg += `${icon('phone', startX, y, iconColor)} <text x="${startX + 35}" y="${y}" font-family="${F}" font-size="18" fill="${textColor}">${esc(d.phone)}</text>`; y += gap; }
  if (d.email) { svg += `${icon('email', startX, y, iconColor)} <text x="${startX + 35}" y="${y}" font-family="${F}" font-size="18" fill="${textColor}">${esc(d.email)}</text>`; y += gap; }
  if (d.website) { svg += `${icon('web', startX, y, iconColor)} <text x="${startX + 35}" y="${y}" font-family="${F}" font-size="18" fill="${textColor}">${esc(d.website)}</text>`; y += gap; }
  if (d.address) { svg += `${icon('loc', startX, y, iconColor)} <text x="${startX + 35}" y="${y}" font-family="${F}" font-size="18" fill="${textColor}">${esc(d.address)}</text>`; }
  return svg;
}
...
const backLayouts = {
  ...
  // 1. 심플: 이름+직함 좌상단, BRAND 우상단, QR 좌하단, 연락처 우하단
  async simple(d) {
    const q = await qr(d.qrUrl, 70, 330, 180);
    return `<svg width="1044" height="590" xmlns="http://www.w3.org/2000/svg">
      <text x="80" y="110" font-family="${F}" font-size="52" font-weight="bold" fill="#111" letter-spacing="10">${esc(d.name)}</text>
      <text x="84" y="150" font-family="${F}" font-size="22" fill="#666">${esc(d.title)}</text>
      <text x="964" y="100" font-family="${F}" font-size="34" font-weight="bold" fill="#111" text-anchor="end">${esc(d.company)}</text>
      ${contactBlock(d, 560, 300)}
      ${q}
    </svg>`;
  },

  // 2. 모던: QR 좌측 대형, 직함+이름 우상단, 연락처 우측
  async modern(d) {
    const q = await qr(d.qrUrl, 60, 80, 240);
    return `<svg width="1044" height="590" xmlns="http://www.w3.org/2000/svg">
      ${q}
      <text x="520" y="100" font-family="${F}" font-size="20" fill="#666">${esc(d.title)}</text>
      <text x="620" y="150" font-family="${F}" font-size="52" font-weight="bold" fill="#111" letter-spacing="10">${esc(d.name)}</text>
      ${contactBlock(d, 560, 280)}
    </svg>`;
  },

  // 3. 서클: 이름+직함 좌상단, 연락처 좌하단, QR 우상단, BRAND 우상단
  async circle(d) {
    const q = await qr(d.qrUrl, 770, 100, 180);
    return `<svg width="1044" height="590" xmlns="http://www.w3.org/2000/svg">
      <text x="80" y="110" font-family="${F}" font-size="52" font-weight="bold" fill="#111" letter-spacing="10">${esc(d.name)}</text>
      <text x="84" y="150" font-family="${F}" font-size="22" fill="#666">${esc(d.title)}</text>
      <text x="964" y="70" font-family="${F}" font-size="30" font-weight="bold" fill="#111" text-anchor="end">${esc(d.company)}</text>
      ${contactBlock(d, 80, 280)}
      ${q}
    </svg>`;
  },

  // 4. 앵글: QR 좌상단, 이름+직함 우상단, 연락처 우측, BRAND 좌하단
  async angle(d) {
    const q = await qr(d.qrUrl, 60, 70, 190);
    return `<svg width="1044" height="590" xmlns="http://www.w3.org/2000/svg">
      ${q}
      <text x="520" y="120" font-family="${F}" font-size="52" font-weight="bold" fill="#111" letter-spacing="10">${esc(d.name)}</text>
      <text x="524" y="160" font-family="${F}" font-size="22" fill="#666">${esc(d.title)}</text>
      ${contactBlock(d, 520, 260)}
      <text x="80" y="500" font-family="${F}" font-size="26" font-weight="bold" fill="#FFF">${esc(d.company)}</text>
    </svg>`;
  },

  // 5. 카드: BRAND+직함+이름 상단(다크), QR 좌하단, 연락처 우하단
  async card(d) {
    const q = await qr(d.qrUrl, 140, 340, 170);
    return `<svg width="1044" height="590" xmlns="http://www.w3.org/2000/svg">
      <text x="120" y="155" font-family="${F}" font-size="30" font-weight="bold" fill="#FFF" letter-spacing="3">${esc(d.company)}</text>
      <text x="480" y="150" font-family="${F}" font-size="18" fill="rgba(255,255,255,0.6)">${esc(d.title)}</text>
      <text x="560" y="155" font-family="${F}" font-size="48" font-weight="bold" fill="#FFF" letter-spacing="10">${esc(d.name)}</text>
      ${contactBlock(d, 530, 370, '#333', '#333')}
      ${q}
    </svg>`;
  },

  // 6. 라인아트: 이름+직함 중앙상단, 연락처 중앙(아이콘), QR 우하단
  async line(d) {
    const q = await qr(d.qrUrl, 780, 300, 170);
    return `<svg width="1044" height="590" xmlns="http://www.w3.org/2000/svg">
      <text x="200" y="130" font-family="${F}" font-size="52" font-weight="bold" fill="#111" letter-spacing="10">${esc(d.name)}</text>
      <text x="204" y="170" font-family="${F}" font-size="22" fill="#666">${esc(d.title)}</text>
      ${contactBlock(d, 100, 270)}
      ${q}
    </svg>`;
  },
};
